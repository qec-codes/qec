...
def test_load_css_code(tmp_path):
    ...
    assert np.all(instance.x_stabilizer_matrix.toarray() == test_css_code.x_stabilizer_matrix.toarray())
    assert np.all(instance.z_stabilizer_matrix.toarray() == test_css_code.z_stabilizer_matrix.toarray())
    assert np.all(instance.x_logical_operator_basis.toarray() == test_css_code.x_logical_operator_basis.toarray())
    assert np.all(instance.z_logical_operator_basis.toarray() == test_css_code.z_logical_operator_basis.toarray())
...
def test_load_hgp_code(tmp_path):
    ...
    assert np.all(instance.seed_matrix_1.toarray() == test_hgp_code.seed_matrix_1.toarray())
    assert np.all(instance.seed_matrix_2.toarray() == test_hgp_code.seed_matrix_2.toarray())
    assert np.all(instance.x_logical_operator_basis.toarray() == test_hgp_code.x_logical_operator_basis.toarray())
    assert np.all(instance.z_logical_operator_basis.toarray() == test_hgp_code.z_logical_operator_basis.toarray())
