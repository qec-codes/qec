...
def get_row_col_data_indices_binary_nonzero(sparse_mat: scipy.sparse.csr_matrix) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    for i in range(len(sparse_mat.indptr) - 1):
        mask = (sparse_mat.indptr[i] <= nonzero_indices) & (nonzero_indices < sparse_mat.indptr[i + 1])
        row_indices[mask] = i
