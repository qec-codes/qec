...
class HypergraphProductCode(CSSCode):
    ...
    def _stabilizer_schedule(self) -> list[Tuple[list[int, int], str]]:
        ...
        of the seed code's Tanner graph and assign signs (cardinal directions) to the
        ...
        num_sector_I = self._n1 * self._n2     # sector layout:  
        num_sector_II = self._m1 * self._n2    # III - Z checks |  IV - data
        num_sector_III = self._n1 * self._m2   # ---------------|--------------
        num_sector_IV = self._m1 * self._m2    #   I - data     | II - X checks
        ...
        for edge in seed_1_ordered:
            ...
            for i in range(self._n2): # I -> II
                target_graph.add_edge(data * self._n2 + i,
                                    (check - self._n1) * self._n2 + i + num_data, 1) 
    
            for i in range(self._m2): # IV -> III
                alternative_graph.add_edge(data * self._m2 + i + num_data + num_sector_II,
                                        (check - self._n1) * self._m2 + i + num_sector_I, 1) 
        
        for check, data in seed_2_tanner.edge_list():

            for i in range(self._n1): # I -> III
                north_south_tanner.add_edge(data + self._n2 * i,
                                            (check - self._n2) + self._m2 * i + num_data + num_sector_II, 1)
            for i in range(self._m1): # IV -> II
                north_south_tanner.add_edge(data + self._n2 * i + num_data,
                                            (check - self._n2) + self._m2 * i + num_sector_I , 1)
