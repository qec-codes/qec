...
class CodeTablesDE(StabiliserCode):
    """
    A code object built from data obtained from Markus Grassl's codetables.de website (with `q=4`).

    This class inherits from `StabiliserCode` and initialises the code
    by querying the codetables.de website for the specified parameters `(n, k)`,
    constructing the stabiliser (PCM) matrix, and passing it up to the
    `StabiliserCode` parent class.
    ...
    See Also
    --------
    StabiliserCode : Parent class providing stabiliser code functionality.
    ...
    def __init__(self, n: int, k: int):
        ...
        This method queries the codetables.de database for a stabiliser matrix
        ...
        the parent `StabiliserCode` class.
        ...
        - `d_upper` from the query result is used to potentially update `self.d`
          if it is smaller than the default distance assigned by `StabiliserCode`.
        ...
        # Construct the stabiliser matrix in CSR format
        # The matrix is 2*n columns wide, as is typical for GF(4) stabilisers.
        stabiliser_matrix = pcm_to_csr_matrix(
            ct_dict["pcm"], num_cols=2 * int(ct_dict["n"])
        )

        # Initialise the parent class with this stabiliser matrix
        super().__init__(stabilisers=stabiliser_matrix)
